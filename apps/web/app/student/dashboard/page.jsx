  ...
  return (
    <AppShell title="Student Dashboard">
      <div className="grid grid-cols-1 gap-4">
        ...
        {/* Departments */}
        <Card title="Departments" description={loading ? "Loading…" : `${departments.length} department(s)`}>
          {loading ? (
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
              {departments.map((d) => (
                <div key={d.id} className="rounded-lg border border-green-100 bg-gradient-to-br from-green-50 to-green-100 p-4 hover:shadow-md transition">
                  <h3 className="font-semibold text-slate-900">{d.name}</h3>
                  <p className="text-sm text-slate-600">Building: {buildingMap.get(d.buildingId) || "N/A"}</p>
                  {d.hodName && <p className="text-sm text-green-700 font-medium">HOD: {d.hodName}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        {/* Rooms */}
        <Card title="Available Rooms & Classrooms" description={loading ? "Loading…" : `${rooms.length} room(s)`}>
          {loading ? (
